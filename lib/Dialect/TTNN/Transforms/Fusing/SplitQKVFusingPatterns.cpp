...
identifyQKVRoles(SmallVector<SliceReshapeMatch> &matches) {
  ...
  for (auto &m : matches) {
    ...
    if (!role) {
      llvm::outs() << "No role found for match\n";
      return std::nullopt;
    }

    uint8_t bit = 1u << static_cast<uint8_t>(*role);
    if (seenMask & bit) {
      llvm::outs() << "Role already seen\n";
      return std::nullopt;
    }
    seenMask |= bit;
...
// Match all matmul users as slice → reshape → (optional permute [0,2,1,3])
// chains. Returns nullopt if any user doesn't match the expected structure.
...
bool validateSliceCoverage(MatMulOpType matmulOp,
  ...
  // Validate contiguous coverage of the full dimension.
  int64_t prevEnd = 0;
  ...
  if (prevEnd != matmulShape[*sliceDim]) {
    return false;
  }
...
//   BHSD: [B*S, H*D] -> [B, H, S, D]  (only valid when S=1, since a dim-of-1
//                                       can be placed anywhere without
//                                       reordering data)
bool validateReshapeLayouts(SmallVector<SliceReshapeMatch> &matches) {
  for (auto &m : matches) {
    ...
    if (isBSHD) {
      // Reshape produces [B,S,H,D] — permute [0,2,1,3] is required.
      if (!m.permuteOp) {
        return false;
      }
    } else if (isBHSD) {
      // Direct reshape to [B,H,S,D] is only valid when S=1.
      if (reshapeShape[2] != 1) {
        return false;
      }
    } else {
...
mlir::LogicalResult createFusedOp(mlir::PatternRewriter &rewriter,
  ...
  int64_t totalHiddenSize = matmulOp.getType().getShape().back();

  bool isGQA = (q.numHeads() != k.numHeads());

  rewriter.setInsertionPointAfter(matmulOp);

  SmallVector<int64_t> inputReshapeShape = {batchSize, seqLen, totalHiddenSize};
...
SplitQueryKeyValueAndSplitHeadsFusing<MatMulOpType>::matchAndRewrite(
  ...
  if (!matches || matches->size() != 3) {
    llvm::outs() << "No matches found\n";
    return mlir::failure();
  }

  // Validate slices are contiguous and cover the full hidden dim.
  if (!validateSliceCoverage(matmulOp, *matches)) {
    llvm::outs() << "Slice coverage validation failed\n";
    return mlir::failure();
  }

  // Validate each reshape is a valid dimension split.
  if (!validateReshapeLayouts(*matches)) {
    llvm::outs() << "Reshape layout validation failed\n";
    return mlir::failure();
  }
  ...
  if (!q || !k || !v || !validateQKVDimensions(*q, *k, *v)) {
    llvm::outs() << "Q/K/V dimension validation failed\n";
    return mlir::failure();
  }
