...
public:
  ...
  matchAndRewrite(ConcreteOp op, typename ConcreteOp::Adaptor adaptor,
    ...
    if constexpr (std::is_same_v<SFPUOp, ttkernel::LogicalNotTileOp>) {
      ...
      auto loc = op->getLoc();
      auto minFloat = arith::ConstantOp::create(
          rewriter, loc,
          rewriter.getF32FloatAttr(op.getMin().convertToFloat()));
      auto maxFloat = arith::ConstantOp::create(
          rewriter, loc,
          rewriter.getF32FloatAttr(op.getMax().convertToFloat()));
      auto minParam = arith::BitcastOp::create(rewriter, loc,
                                               rewriter.getI32Type(), minFloat);
      auto maxParam = arith::BitcastOp::create(rewriter, loc,
                                               rewriter.getI32Type(), maxFloat);
